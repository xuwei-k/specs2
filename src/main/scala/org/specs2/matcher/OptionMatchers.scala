  ...
  def which(f: T => Boolean) = new Matcher[Option[T]] {
    def apply[S <: Option[T]](value: Expectable[S]) = {
      ...
      val (isSuccess, failureMessage) = value.value match {
        case Some(t) if !f(t) => (false, s"${value.description} is Some[T] but $t does not satisfy given predicate")
        case None             => (false, defaultMessage)
        case _                => (true , defaultMessage)
      }
      result(isSuccess,
             s"${value.description} is Some[T] and satisfy given predicate",
             failureMessage,
             value)
    }
  }
